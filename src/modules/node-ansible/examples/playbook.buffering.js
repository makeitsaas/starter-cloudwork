...
command.on('stdout', function (data) {
    console.log("stdout unb: " + data.toString());
});
command.on('stderr', function (data) {
    console.log("stderr unb: " + data.toString());
});
var promise = command.exec({buffered: false});

var commandb = new Playbook().playbook(playbook_file);
commandb.inventory(inventory_file);
commandb.on('stdout', function (data) {
    console.log("stdout buf: " + data.toString());
});
commandb.on('stderr', function (data) {
    console.log("stderr buf: " + data.toString());
});
